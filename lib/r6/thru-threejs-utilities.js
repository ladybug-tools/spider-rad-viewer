...
THRU.setHelpers = function() {
	...
	THR.renderer.domElement.addEventListener( 'click', () => THRU.sceneRotation = 0, false );
...
THRU.addSomeLights = function() {
	...
	THR.scene.add( new THREE.AmbientLight( 0x444444 ) );

	THRU.light1 = new THREE.DirectionalLight( 0xffffff, 0.5 );
	THRU.light1.position.set( 1, 1, 1 );
	THR.scene.add( THRU.light1 );

	THRU.light2 = new THREE.DirectionalLight( 0xffffff, 1.5 );
	THRU.light2.position.set( 0, -1, 0 );
...
// supersedes animate in thr-three.js
...
THR.animate = function() {
	...
	THR.controls.update();
	THR.scene.rotation.z += THRU.sceneRotation / 1000;
