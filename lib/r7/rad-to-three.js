...
RAD.getTrianglesMesh = function( vertices, colors ) {
	...
	const materialTriangles = new THREE.MeshPhongMaterial( { color: 0xaaaaaa, side: 2, vertexColors: THREE.VertexColors } );
...
RAD.setShapeMesh = function( polygon ) {
	...
	const color = RAD.getColor( polygon );

	//const material = new THREE.MeshNormalMaterial( {  opacity: RAD.opacity, side: 2, transparent: true } );
	const material = new THREE.MeshPhongMaterial( { color: color, opacity: RAD.opacity, side: 2, transparent: true } );
...
RAD.drawCylinder = function( cylinder ) {
	...
	const color = RAD.getColor( cylinder );
...
RAD.drawSphere = function( sphere ) {
	...
	const color = RAD.getColor( sphere );
...
RAD.getColor = function( geometry ){
	...
	RAD.triangleMaterials.push( colorText );
