...
THRU.getSettings = function() {
	...
	`
		...
		<p><button onclick=THRU.zoomObjectBoundingSphere(THR.scene);>zoom all</button></p>
...
THRU.toggleSurfaceNormals = function() {
	...
	const geometryNormal = new THREE.BoxGeometry( 10, 10, 10, 2, 2, 2 );
	const materialNormal = new THREE.MeshBasicMaterial( { color: 0xff0000 } );
	const box = new THREE.Mesh( geometryNormal, materialNormal );
	const helper = new THREE.FaceNormalsHelper( box, 2, 0x00ff00, 1 );
	//THR.scene.add( box );
...
THRU.getSomeBoxes = function( count = 50, size = 10 ) {
